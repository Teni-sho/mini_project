...
resource "aws_ib_target_group" "min-target-group" {
...
resource "aws_ib_listener_rule" "min-listener-rule" {
...
resource "aws_ib_target_group_attachment" "min-target-group-attachment1" {
    target_group_arn = aws_lb_target_group.min-target-group.arn
    target_id = aws_instance.min-1.id 
    port = 80 
}

resource "aws_ib_targer_group_attachment" "min-target-group-attachment2" {
    target_group_arn = aws_lb_target_group.min-target-group.arn
    target_id = aws_instance.min-2.id 
    port = 80 
}

resource "aws_ib_targer_group_attachment" "min-target-group-attachment3" {
    target_group_arn = aws_lb_target_group.min-target-group.arn
